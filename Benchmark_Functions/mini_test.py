...
from utils.arabic_support import setup_arabic_fonts
...
def run_mini_test():
    ...
    for algorithm_name, result in results.items():
        plt.semilogy(result["convergence_curve"], label=algorithm_name)
    
    plt.title(f"منحنيات التقارب للدالة {function_name} (اختبار مصغر)")
    plt.xlabel("التكرار")
    ...
    # حفظ الرسم البياني
    plt.savefig(f"{PLOTS_DIR}/mini_test_convergence.{PLOT_FORMAT}")
    plt.close()
    
    print(f"تم حفظ منحنيات التقارب في {PLOTS_DIR}/mini_test_convergence.{PLOT_FORMAT}")
