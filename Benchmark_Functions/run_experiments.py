...
from utils.arabic_support import setup_arabic_fonts, create_arabic_boxplot, create_arabic_convergence_plot
...
def run_all_experiments():
    ...
    for algorithm_name in ALGORITHMS:
        ...
        for function_name in FUNCTIONS:
            ...
            plt.semilogy(mean_convergence)
            plt.title(f"{algorithm_name} على {function_name} - منحنى التقارب")
            plt.xlabel("التكرار")
            plt.ylabel("أفضل قيمة (مقياس لوغاريتمي)")
            plt.grid(True)
            plt.savefig(f"{PLOTS_DIR}/{algorithm_name}_{function_name}_convergence.{PLOT_FORMAT}")
            plt.close()
